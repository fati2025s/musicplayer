...
class SocketService {
    ...
    try {
      ...
      _socket!.listen((data) {
        ...
        for (final part in str.split('\n')) {
          ...
          try {
            final jsonResp = jsonDecode(trimmed) as Map<String, dynamic>;
            ...
            print('Error parsing response: $e | raw: $trimmed');
          }
        }
      }, onDone: () {
        _connected = false;
  ...
  Future<void> downloadSong(int songId, String savePath) async {
    await _sendWithAuth('downloadSong', {'songId': songId});
  ...
  Future<Map<String, dynamic>> likeSong(int songId) async =>
      await _sendWithAuth('likeSong', {'songId': songId});

  Future<Map<String, dynamic>> unlikeSong(int songId) async =>
      await _sendWithAuth('unlikeSong', {'songId': songId});
  ...
  Future<Map<String, dynamic>> uploadProfilePicture(String filePath) async {
    ...
    return await _sendWithAuth('uploadProfilePicture', {
      'fileName': file.uri.pathSegments.last,
      'base64Data': base64Data,
    });
  }
  ...
  Future<Map<String, dynamic>> updateProfile(Map<String, dynamic> changes) async {
    return await _sendWithAuth('updateProfile', changes);
  }
